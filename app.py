...
for file in os.listdir(g['DATA_FINAL_PATH']):
    ...
    if '_Daily' in file:
        try:
            ...
            cur.execute("\\COPY stg_daily_trades(trader,sequence_no,account,side,symbol,quantity,price,destination,contra,trade_datetime,bo_account,cusip,liq,order_id,exec_broker,ecn_fee,order_datetime,specialist,commission,bb_trade,sec_fee,batch_id,client_order_id,prime,cover_quantity,userr,settle_date,principal,net_amount,allocation_id,allocation_role,is_clearable,nscc_fee,nasdaq_fee,clearing_fee,nyse_etf_fee,amex_etf_fee,listing_exchange,native_liq,order_received_id,bo_group_id) FROM \'%(q_file_path)s\' WITH CSV HEADER DELIMITER \',\'", {'q_file_path':'{0}{1}'.format(g['DATA_FINAL_PATH'], file)})
